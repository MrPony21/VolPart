...
// Constructor del documento impreso. La factura y la cotizacion son la misma
// hoja con distinto encabezado, asi que el layout vive aqui una sola vez:
// duplicarlo obligaba a arreglar dos veces cualquier ajuste de columnas.
...
// ── Helpers ──────────────────────────────────────────────────────────────────
// Exportados porque la cotización arma su propia hoja, con otro diseño, pero
// pinta con las mismas primitivas.
export function setFill(doc, color)   { doc.setFillColor(color[0], color[1], color[2]); }
export function setStroke(doc, color) { doc.setDrawColor(color[0], color[1], color[2]); }
export function setTxt(doc, color)    { doc.setTextColor(color[0], color[1], color[2]); }
...
/**
 * Genera y descarga un documento (factura o cotización) en PDF.
 *
 * @param {object} params
 * @param {string} params.titulo        - Encabezado, ej. 'FACTURA DE VENTA'
 * @param {string} params.etiquetaSerie - Rótulo del número, ej. 'N° Serie'
 * @param {string} params.serie         - Número del documento
 * @param {object} params.cliente       - { nit, nombre, telefono, direccion }
 * @param {Array}  params.items         - [{ codigo, descripcion, cantidad, precio }]
 * @param {number} params.total         - Total del documento
...
export function generarDocumentoPDF({
  ...
  doc.setFontSize(20);
  doc.text(titulo, PW - M, 13, { align: 'right' });

  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text(`${etiquetaSerie}: ${serie}`, PW - M, 20, { align: 'right' });
  ...
  items.forEach((item, idx) => {
    ...
    const cellData = [
      ...
      { val: String(item.codigo ?? '-'),                 col: cols[1] },
  ...
  doc.setFontSize(8);
  doc.text(leyendaPie, PW / 2, footerY + 5, { align: 'center' });
